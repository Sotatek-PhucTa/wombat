...
import { IUSD_POOL_TOKENS_MAP } from '../tokens.config'
import { confirmTxn, logVerifyCommand } from '../utils'

const contractName = 'IUSDPoolAsset'
...
const deployFunc: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(`Step 023. Deploying on : ${hre.network.name} with account : ${deployer}`)

  // create asset contracts, e.g. LP-USDC, LP-BUSD, etc. for the ERC20 stablecoins list
  const IUSD_POOL_TOKENS = IUSD_POOL_TOKENS_MAP[hre.network.name]
  ...
  for (const index in IUSD_POOL_TOKENS) {
    console.log('Attemping to deploy Asset contract : ' + IUSD_POOL_TOKENS[index][0])
    const tokenSymbol = IUSD_POOL_TOKENS[index][1] as string
    const tokenName = IUSD_POOL_TOKENS[index][0] as string

    const tokenAddress: string =
      hre.network.name == 'bsc_mainnet'
        ? (IUSD_POOL_TOKENS[index][2] as string)
    ...
    const usdAssetDeployResult = await deploy(`Asset_IUSDPool_${tokenSymbol}`, {
      from: deployer,
      contract: 'Asset',
      log: true,
...
deployFunc.dependencies = ['IUSDPool'] // this ensure the Token script above is executed first, so `deployments.get('SidePool_01')` succeeds
