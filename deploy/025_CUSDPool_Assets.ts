...
import { CUSD_POOL_TOKENS_MAP } from '../tokens.config'
import { confirmTxn, logVerifyCommand } from '../utils'

const contractName = 'CUSDPoolAsset'
...
const deployFunc: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const CUSD_POOL_TOKENS = CUSD_POOL_TOKENS_MAP[hre.network.name]
  ...
  for (const index in CUSD_POOL_TOKENS) {
    console.log('Attemping to deploy Asset contract : ' + CUSD_POOL_TOKENS[index][0])
    const tokenSymbol = CUSD_POOL_TOKENS[index][1] as string
    const tokenName = CUSD_POOL_TOKENS[index][0] as string

    const tokenAddress: string =
      hre.network.name == 'bsc_mainnet'
        ? (CUSD_POOL_TOKENS[index][2] as string)
    ...
    const usdAssetDeployResult = await deploy(`Asset_CUSDPool_${tokenSymbol}`, {
      from: deployer,
      contract: 'Asset',
      log: true,
...
deployFunc.dependencies = ['CUSDPool'] // this ensure the Token script above is executed first, so `deployments.get('SidePool_01')` succeeds
