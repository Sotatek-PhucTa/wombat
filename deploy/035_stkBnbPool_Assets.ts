...
import { STKBNB_POOL_TOKENS_MAP } from '../tokens.config'
import { logVerifyCommand } from '../utils'

const contractName = 'StkBnbPool_Assets'
const poolContractName = 'StkBnbPool'
const assetContractPrefix = 'Asset_StkBnbPool_'
...
const deployFunc: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(`Step 035. Deploying on : ${hre.network.name} with account : ${deployer}`)
...
/** @todo refactor it under utils/deploy.ts */
export async function deployAsset(
  ...
  console.log(`Attemping to deploy ${assetContract} contract: ` + tokenName + ', args: ', args)
  ...
  const asset = await ethers.getContractAt(assetContract, address)
