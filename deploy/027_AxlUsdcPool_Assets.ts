...
import { AXLUSDC_POOL_TOKENS_MAP } from '../tokens.config'
import { confirmTxn, logVerifyCommand } from '../utils'

const contractName = 'AxlUsdcPoolAsset'
...
const deployFunc: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(`Step 027. Deploying on : ${hre.network.name} with account : ${deployer}`)

  // create asset contracts, e.g. LP-USDC, LP-BUSD, etc. for the ERC20 stablecoins list
  const AXLUSDC_POOL_TOKENS = AXLUSDC_POOL_TOKENS_MAP[hre.network.name]
  ...
  for (const index in AXLUSDC_POOL_TOKENS) {
    console.log('Attemping to deploy Asset contract : ' + AXLUSDC_POOL_TOKENS[index][0])
    const tokenSymbol = AXLUSDC_POOL_TOKENS[index][1] as string
    const tokenName = AXLUSDC_POOL_TOKENS[index][0] as string

    const tokenAddress: string =
      hre.network.name == 'bsc_mainnet'
        ? (AXLUSDC_POOL_TOKENS[index][2] as string)
    ...
    const usdAssetDeployResult = await deploy(`Asset_AxlUsdcPool_${tokenSymbol}`, {
      from: deployer,
      contract: 'Asset',
      log: true,
...
deployFunc.dependencies = ['AxlUsdcPool'] // this ensure the Token script above is executed first, so `deployments.get('SidePool_01')` succeeds
