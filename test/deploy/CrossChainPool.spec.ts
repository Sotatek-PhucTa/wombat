...
describe('CrossChainPool deployment', function () {
  ...
  describe('Verify pool config', function () {
    ...
    it(`verify config for ${contractName}`, async function () {
      ...
      expect(await pool.tokensForCreditHaircut()).to.eq(parseEther('0.0001'), 'tokens for credit haircut not equal')
      expect(await pool.creditForTokensHaircut()).to.eq(parseEther('0.0001'), 'credit for tokens haircut not equal')
      expect(await pool.maximumInboundCredit()).to.eq(parseEther('10000'), 'maximum inbound credit not equal')
      expect(await pool.maximumOutboundCredit()).to.eq(parseEther('10000'), 'maximum outbound credit not equal')
