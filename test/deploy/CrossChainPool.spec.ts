import {
  getLayerZeroAdaptorDeploymentName,
...
import { CrossChainPool, LayerZeroAdaptor, WormholeAdaptor } from '../../build/typechain'
import { deployments, ethers, getNamedAccounts } from 'hardhat'
import { Token, getTokenDeploymentOrAddress } from '../../config/token'
import { parseEther } from 'ethers/lib/utils'

describe('CrossChainPool deployment', function () {
  const poolNames = ['Stablecoin_Pool', 'LayerZero_Stablecoin_Pool']
  ...
  describe('Verify adaptor config', function () {
    let wormholeAdaptor: WormholeAdaptor
    let layerzeroAdaptor: LayerZeroAdaptor
    const wormholeChainId = 0
    const layerzeroChainId = 65535

    beforeEach(async function () {
      wormholeAdaptor = (await getDeployedContract(
        'WormholeAdaptor',
        getWormholeAdaptorDeploymentName('Stablecoin_Pool')
      )) as WormholeAdaptor
      layerzeroAdaptor = (await getDeployedContract(
        'LayerZeroAdaptor',
        getLayerZeroAdaptorDeploymentName('LayerZero_Stablecoin_Pool')
      )) as LayerZeroAdaptor
    })

    it(`verify address config for WormholeAdaptor`, async function () {
    ...
    it(`verify address config for LayerZeroAdaptor`, async function () {
      // verify add adaptor address
      expect(await layerzeroAdaptor.getTrustedRemoteAddress(layerzeroChainId)).to.eq(
        '0x0000000000000000000000000000000000000002'
      )

      // verify approve token
      expect(
        await layerzeroAdaptor.validToken(layerzeroChainId, await getAddress(getTokenDeploymentOrAddress(Token.BUSD)))
      ).to.be.true
      expect(
        await layerzeroAdaptor.validToken(layerzeroChainId, await getAddress(getTokenDeploymentOrAddress(Token.vUSDC)))
      ).to.be.true
    })

    it('verify wormholeAdaptor and layerzeroAdaptor not connect with each other', async function () {
      expect(await wormholeAdaptor.adaptorAddress(layerzeroChainId)).to.eq('0x0000000000000000000000000000000000000000')
      await expect(layerzeroAdaptor.getTrustedRemoteAddress(wormholeChainId)).to.revertedWith(
        'LzApp: no trusted path record'
      )

      expect(
        await wormholeAdaptor.validToken(layerzeroChainId, await getAddress(getTokenDeploymentOrAddress(Token.BUSD)))
      ).to.be.false
      expect(
        await wormholeAdaptor.validToken(layerzeroChainId, await getAddress(getTokenDeploymentOrAddress(Token.vUSDC)))
      ).to.be.false

      expect(
        await layerzeroAdaptor.validToken(wormholeChainId, await getAddress(getTokenDeploymentOrAddress(Token.BUSD)))
      ).to.be.false
      expect(
        await layerzeroAdaptor.validToken(wormholeChainId, await getAddress(getTokenDeploymentOrAddress(Token.vUSDC)))
      ).to.be.false
    })
  })
})
