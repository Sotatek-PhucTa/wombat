...
describe('MasterWombat', function () {
  ...
  beforeEach(async function () {
    ...
    // 80% of emissions generated in 12 months
    this.womPerSec = parseEther('1000000').mul(8).div(10).div(365).div(24).div(3600)
    this.mockERC20 = await this.MockERC20.connect(owner).deploy('mock', 'MOCK', 18, parseEther('100'))
    this.veWom = await this.MockERC20.connect(owner).deploy('vewom', 'vewom', 18, parseEther('100'))
    this.dummyToken = await this.MockERC20.connect(owner).deploy('Dummy', 'DUMMY', 6, parseUnits('100', 6))
  })
  ...
  describe('Master Wombat Utils', function () {
    ...
    it('should give out woms only after farming time', async function () {
      ...
      expect((await latest()).toNumber()).to.lt(await this.mw.startTimestamp())

      // try to claim
      await this.mw.connect(users[1]).deposit(0, '0')
      expect(await this.wom.balanceOf(users[1].address)).to.equal('0')

      // advance time beyond start time
      await advanceTimeAndBlock(100)
      expect((await latest()).toNumber()).to.gt(await this.mw.startTimestamp())
      await this.mw.connect(users[1]).deposit(0, '0')
    ...
    it('should not distribute woms if no one deposit', async function () {
      ...
      expect(pendingTokens).to.be.gt(0)
      ...
      expect(await this.wom.balanceOf(users[1].address)).to.be.gt(pendingTokens)
    })
  })

  describe('[All pools] Emission', function () {
    beforeEach(async function () {
      // We make start time 60 seconds past the last block
      const startTime = (await latest()).add(60)
      ...
      await this.mw.initialize(this.wom.address, this.veWom.address, this.womPerSec, startTime)

      // transfer 80% wom supply for emission
    ...
    it('allocate tokens correctly', async function () {
      const [pendingTokens0] = await this.mw.connect(users[1]).pendingTokens(0, users[1].address)
      const [pendingTokens1] = await this.mw.connect(users[1]).pendingTokens(1, users[1].address)
      const [pendingTokens2] = await this.mw.connect(users[1]).pendingTokens(2, users[1].address)
      const [pendingTokens3] = await this.mw.connect(users[1]).pendingTokens(3, users[1].address)

      const sum = pendingTokens0.add(pendingTokens1).add(pendingTokens2).add(pendingTokens3)
      expect(pendingTokens0).to.be.near(sum.mul(30).div(100))
      expect(pendingTokens1).to.be.near(sum.mul(30).div(100))
      expect(pendingTokens2).to.be.near(sum.mul(25).div(100))
      expect(pendingTokens3).to.be.near(sum.mul(15).div(100))
    })

    it('withdraw should claim wom', async function () {
      expect(await this.wom.balanceOf(users[1].address)).to.eq(0)
      await this.mw.connect(users[1]).withdraw(0, 0)
      expect(await this.wom.balanceOf(users[1].address)).to.gt(0)
    })

    it('deposit should claim wom', async function () {
      expect(await this.wom.balanceOf(users[1].address)).to.eq(0)
      await this.mw.connect(users[1]).deposit(0, 0)
      expect(await this.wom.balanceOf(users[1].address)).to.gt(0)
    })
    ...
    describe('multiclaim', function () {
      it('one pool only', async function () {
        await this.mw.connect(users[1]).multiClaim([3])
        expect(await this.wom.balanceOf(users[1].address)).to.near(parseEther('119999.863'))
      })

      it('two pools then two pools', async function () {
        await this.mw.connect(users[1]).multiClaim([0, 1])
        expect(await this.wom.balanceOf(users[1].address)).to.near(parseEther('479999.452054794516698300'))

        await this.mw.connect(users[1]).multiClaim([2, 3])
        expect(await this.wom.balanceOf(users[1].address)).to.near(parseEther('799999.086734794516698300'))
      })

      it('all pools', async function () {
        await this.mw.connect(users[1]).multiClaim([0, 1, 2, 3])
        expect(await this.wom.balanceOf(users[1].address)).to.near(parseEther('799999.086734794516698300'))
      })
    })
  })
